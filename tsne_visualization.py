...
def main(config):
    ...
    model.eval()
    for idx, (images, target) in enumerate(tqdm(data_loader_val)):
        ...
        embeddings = np.concatenate([embeddings, feature.detach().cpu().numpy()], axis=0)
    ...
    plt.savefig(os.path.join(config.OUTPUT, 'tsne.png'))
